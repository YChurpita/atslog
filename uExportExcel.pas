...
const
	...
	begin
		...
		V.WorkBooks[1].Worksheets[1].Name := 'Выборка';
		Sheet := V.WorkBooks[1].Worksheets[ 'Выборка' ];
		...
		if (StatusBar<>nil) then begin
			StatusBar.SimpleText := ' Экспортировано ' + IntToStr( SheetRowCnt - 1 )+' аозиций в Excel';
		end;
...
begin
	...
	if (trim(uppercase(frmFieldSelection.ExportLimitCB.Text))<>'ALL') then begin
		try
			try
				...
				ShowMessage('Непонятная ошибка при эспорте!');
				Ok := False;
			end;
		finally
		end;
	...
	if ok then begin
      ...
      fWait.Show('Экспортирование в эксель');
      Application.ProcessMessages;
